...
class Board:
    ...
    def calculate_points(self, input):
        print('POINTS:', list(self.values.keys()), input)
...
with open('input.txt') as f:
    ...
    for input in inputs:
        ...
        for i, b in enumerate(boards):
            if not done[i] and b.play(input):
                done[i] = True
                totalDone += 1
                print(input, i, totalDone, len(boards), b.lines)
                if totalDone == len(boards):
                    print(b.getPoints())
                    exit()
            # else:
                print(b.lines)