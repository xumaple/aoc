...
#### expand map
def increase_list(l, n):
    return [(x+n-1)%9+1 for x in l]
    # for i in range(n):

map = [list(chain(*[increase_list(l, i) for i in range(EXPAND_TIMES)])) for l in map]
le = len(map)
for i in range(1, EXPAND_TIMES):
    for _, l in enumerate(map[:le]):
        map.append(increase_list(l, i))
####
